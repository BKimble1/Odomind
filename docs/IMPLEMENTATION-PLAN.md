...
7. **A way to see the app without a Mac.** Artifact downloads are not reachable
   from every environment this was built in, so the `Screenshots` workflow also
   writes each capture into the log of its own small job, base64, for
   `decode-screenshot.py` to reassemble. Until it existed, nothing here had
   been *looked at* — and the first look found five layout defects in one
   screen. It is manual only and never a gate.
8. **An identifier cross-check that runs in seconds.** A UI test waiting on an
   accessibility identifier the app never sets does not fail fast: it builds,
   boots a simulator, waits out its timeout, and reports half an hour later.
   Twice that happened here. A text search answers it immediately, so it runs
   on the Ubuntu job.
