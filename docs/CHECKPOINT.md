...
Build 2 is the current milestone. `docs/BUILD-2-NOTES.md` has the full feature
status table and the tester checklist; this is the engineering state.
...
| Screens | Build 2: onboarding, Home, Jobs, Garage, Calendar, Parts, Pro, Settings. Today, Maintenance and History are gone; their files were deleted rather than left orphaned. |
...
| TestFlight | Shipping. `testflight.yml` built, signed and uploaded 1.0 (6); Apple processed it to VALID. |
